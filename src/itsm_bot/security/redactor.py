"""Маскирование секретов и персональных данных до записи заявки.

Единственный рубеж защиты. Всё, что идёт дальше по конвейеру — запись в БД, карточка
в канале исполнителя, логи, уведомления — работает только с результатом этой функции;
сырой текст живёт в памяти обработчика и никуда не сохраняется.

Правила намеренно осторожные: ложное срабатывание уродует описание заявки и мешает
исполнителю (стереть код ошибки хуже, чем пропустить сомнительную строку). Цена этой
осторожности — пропущенное здесь пропущено навсегда: второй рубеж, который раньше
отводился модели, архитектурой не предусмотрен (решение D1 в TODO.md).
"""
